...
This mode will only concatenate files that contain the exact number of columns as well as matching column names (case and space sensitive). Any files that do not meet this stringent requirements will be removed from the final concatenated file. Furthermore, the filename will be provided so that the user can correct this file. The argument `--strict` is necessary for this mode to run, no other value is required.
